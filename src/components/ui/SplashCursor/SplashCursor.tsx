...
interface SplashCursorProps {
  ...
  COLOR?: string;
  /** When true the simulation loop skips step/applyInputs — keeps GPU free for other WebGL work.
   *  render() still runs every frame so the WebGL compositing layer is never discarded. */
  paused?: boolean;
  /** When true the canvas wrapper fades to opacity 0. Set from inside SplashCursor's own
   *  position:fixed div so the opacity is on the correct stacking-context element. */
  hidden?: boolean;
}
...
export default function SplashCursor({
  ...
  // Refs so the updateFrame closure always reads the live values without
  // needing to be listed in the deps array.
  ...
  useEffect(() => {
    ...
    // Frame counter for throttled keep-alive renders while paused.
    ...
    const BRAND_PALETTE = ['#5EEAD4', '#60A5FA', '#A78BFA'];
    function generateColor() {
      if (!config.RAINBOW_MODE) {
        const hex = BRAND_PALETTE[Math.floor(Math.random() * BRAND_PALETTE.length)];
        return hexToRGB(hex);
      }
    ...
    function handleMouseMove(e: MouseEvent) {
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(e.clientX); const posY = scaleByPixelRatio(e.clientY);
  ...
  return (
    // opacity is set HERE (on the fixed div that owns the stacking context)
    // rather than on a wrapper — a zero-size wrapper's opacity does not
    // reliably propagate to position:fixed children in all browsers.
    <div
      style={{
