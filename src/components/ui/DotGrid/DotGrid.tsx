...
if (typeof window !== 'undefined') {
  import('gsap/InertiaPlugin')
    .then(({ InertiaPlugin }) => gsap.registerPlugin(InertiaPlugin))
    .catch(() => {});
}
...
const DotGrid = ({
  ...
  const buildGrid = useCallback(() => {
    ...
    // Use offsetWidth/offsetHeight — these return the LAYOUT size and are NOT
    // affected by CSS transform: scale() on an ancestor. This means the canvas
    // always fills the section at its true dimensions regardless of zoom level.
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      // Convert visual (post-transform) mouse coords to layout (pre-transform) dot coords
      const scaleX = canvas.offsetWidth  / (rect.width  || 1);
      const scaleY = canvas.offsetHeight / (rect.height || 1);
      pr.x = (e.clientX - rect.left) * scaleX;
      pr.y = (e.clientY - rect.top)  * scaleY;
